...
int main(int argc, char const* argv[])
{
    ...
    comm = new Comm(nrank, rank, rank, ip, port);

    pthread_t cudnn, nccl;
    pthread_create(&cudnn, NULL, run_cudnn, NULL);
    pthread_create(&nccl, NULL, run_nccl, NULL);
    
    pthread_join(cudnn, NULL);
    pthread_join(nccl, NULL);
    comm->syncStream();
