...
static void * commReceive(int fd) {
  ...
  while (size_to_receive > 0) {
    ...
    if (bytes_read < 0) printf("read error\n");
    buf += bytes_read;
...
Comm::Comm(int nrank, int rank, int device, const char *ip_addr, uint16_t port) {
  ...
  if (rank == 0) {
    ...
    while (connection_remain > 0) {
      ...
      int remote_fd;
      if ((remote_fd = accept(socket_fd, (sockaddr*)&daddr, &length)) < 0) {
        printf("accept error\n");
      }
